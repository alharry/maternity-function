...
Figure 2. Bias (per cent relative error) in parameter estimates for $\hat{L'_{50}}$ and $\hat{P_{Max}}$ for the 3PLF maternity function with $P_{Max}$ estimated. Each point represents parameter estimates from one iteration of simulated data (n = 43,129), including all combinations of variables. Simulations with longer reproductive cycles and fewer maternal females were associated with higher bias in both parameters $\hat{L'_{50}}$ and $\hat{P_{Max}}$. Note: 42 data points were cropped to aid with data visualization (see Figure S12 for uncropped figure).\
\
\newpage
...
(p <- ggplot(data = data %>% 
  filter(method_long %in% c("3PLF - estimated"), par_name %in% "c"), aes(x = n_maternal, y = abs_error, col = repro_freq)) + 
  geom_point() + 
  scale_color_brewer(palette = "Spectral") +
